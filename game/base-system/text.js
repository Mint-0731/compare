...
statDisplay.create("llove", (npc, simple) => {
	...
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, -1, "red");
});
statDisplay.create("lllove", (npc, simple) => {
	const selectedNPC = npc || V.npc.find(name => !!C.npc[name]);
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, -2, "red");
});
statDisplay.create("llllove", (npc, simple) => {
	const selectedNPC = npc || V.npc.find(name => !!C.npc[name]);
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, -3, "red");
});
statDisplay.create("glove", (npc, simple) => {
	const selectedNPC = npc || V.npc.find(name => !!C.npc[name]);
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, 1, "green");
});
statDisplay.create("gglove", (npc, simple) => {
	const selectedNPC = npc || V.npc.find(name => !!C.npc[name]);
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, 2, "green");
});
statDisplay.create("ggglove", (npc, simple) => {
	const selectedNPC = npc || V.npc.find(name => !!C.npc[name]);
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.npc.length >= 2) ? `${trNPCname(selectedNPC, '의')} ${loveAlias}` : loveAlias, 3, "green");
});
...
statDisplay.create("lperlove", (npc, simple) => {
	...
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, -1, "red");
});
statDisplay.create("llperlove", (npc, simple) => {
	const selectedNPC = npc || V.NPCList[V.index].per;
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	const NPCName = `${(selectedNPC && setup.perName[selectedNPC]()) || ""}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, -2, "red");
});
statDisplay.create("lllperlove", (npc, simple) => {
	const selectedNPC = npc || V.NPCList[V.index].per;
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	const NPCName = `${(selectedNPC && setup.perName[selectedNPC]()) || ""}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, -3, "red");
});
statDisplay.create("gperlove", (npc, simple) => {
	const selectedNPC = npc || V.NPCList[V.index].per;
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	const NPCName = `${(selectedNPC && setup.perName[selectedNPC]()) || ""}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, 1, "green");
});
statDisplay.create("ggperlove", (npc, simple) => {
	const selectedNPC = npc || V.NPCList[V.index].per;
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	const NPCName = `${(selectedNPC && setup.perName[selectedNPC]()) || ""}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, 2, "green");
});
statDisplay.create("gggperlove", (npc, simple) => {
	const selectedNPC = npc || V.NPCList[V.index].per;
	const loveAlias = `${(selectedNPC && setup.loveAlias[selectedNPC]()) || "사랑"}`;
	const NPCName = `${(selectedNPC && setup.perName[selectedNPC]()) || ""}`;
	return statDisplay.statChange(selectedNPC && !simple && (npc || V.enemyno >= 2) ? `${trNPCname(selectedNPC, '의')}` : loveAlias, 3, "green");
});
...
statDisplay.create("gvaginalskill", () => statDisplay.statChange("질 기술", 1, "green"));
statDisplay.create("gpenileskill", () => statDisplay.statChange("페니스 기술", 1, "green"));
...
statDisplay.create("ldom", npc => {
	...
	} else if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
...
statDisplay.create("lfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, -1, "red");
});
statDisplay.create("llfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, -2, "red");
});
statDisplay.create("lllfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, -3, "red");
});
statDisplay.create("gfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, 1, "green");
});
statDisplay.create("ggfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, 2, "green");
});
statDisplay.create("gggfrustration", npc => {
	let targetName = "";
	if (npc) targetName = npc + "의";
	else if (V.npc.length >= 2) targetName = V.npc[0] + "의";
	return statDisplay.statChange(`${trNPCname(targetName, '의')} 불만`, 3, "green");
});
...
statDisplay.create("lyearning", () => {
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", -1, "witchbloom");
	return statDisplay.statChange("???", -1, "witchbloom");
});
statDisplay.create("llyearning", () => {
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", -2, "witchbloom");
	return statDisplay.statChange("???", -2, "witchbloom");
});
statDisplay.create("lllyearning", () => {
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", -3, "witchbloom");
...
statDisplay.create("gyearning", (overrideStage = false) => {
	...
	if (
		...
		return "";
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", 1, "forest-green");
...
statDisplay.create("ggyearning", (overrideStage = false) => {
	...
	if (
		...
		return "";
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", 2, "forest-green");
...
statDisplay.create("gggyearning", (overrideStage = false) => {
	...
	if (
		...
		return "";
	if (V.gwylanSeen?.includes("yearning")) return statDisplay.statChange("동경", 3, "forest-green");
	return statDisplay.statChange("???", 3, "forest-green");
});

statDisplay.create("lworldcorruption", () =>
	statDisplay.statChange("세계 오염도", -1, "gold", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("llworldcorruption", () =>
	statDisplay.statChange("세계 오염도", -2, "gold", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("lllworldcorruption", () =>
	statDisplay.statChange("세계 오염도", -3, "gold", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("gworldcorruption", () =>
	statDisplay.statChange("세계 오염도", 1, "tentacle", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("ggworldcorruption", () =>
	statDisplay.statChange("세계 오염도", 2, "tentacle", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("gggworldcorruption", () =>
	statDisplay.statChange("세계 오염도", 3, "tentacle", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("lworldcorruptionhard", () =>
	statDisplay.statChange("세계의 구멍이 막혀간다...", 0, "gold", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
statDisplay.create("gworldcorruptionhard", () =>
	statDisplay.statChange("세계에 새로운 구멍이 생긴다...", 0, "tentacle", () => V.temple_confessor_intro || V.gwylanSeen?.includes("ritual_beast"))
);
...
statDisplay.create("gwarmth", () => statDisplay.statChange("온기", 1, "orange"));
statDisplay.create("ggwarmth", () => statDisplay.statChange("온기", 2, "orange"));
statDisplay.create("gggwarmth", () => statDisplay.statChange("온기", 3, "orange"));
statDisplay.create("lwarmth", () => statDisplay.statChange("온기", -1, "teal"));
statDisplay.create("llwarmth", () => statDisplay.statChange("온기", -2, "teal"));
statDisplay.create("lllwarmth", () => statDisplay.statChange("온기", -3, "teal"));
