function init_trHands()
{
    setup.trClothes.hands = [
        {
            index: 0,
            name: "naked",
            name_ko: "맨손",
            variable: "naked",
            post: 0,
            description: "naked"
        },

        {
            index: 1,
            name: "fingerless gloves",
            name_ko: "핑거리스 장갑",
            variable: "fingerlessgloves",
            post: 0,
            description: "손을 따뜻하게 하기 위한 핑거리스 장갑."
        },

        {
            index: 2,
            name: "mittens",
            name_ko: "벙어리 장갑",
            variable: "mittens",
            post: 0,
            description: "당신의 손에 꼭 맞는 장갑."
        },

        {
            index: 3,
            name: "arm warmers",
            name_ko: "팔 토시",
            variable: "armwarmers",
            post: 1,
            description: "당신의 팔을 따뜻하게 유지한다."
        },

        {
            index: 4,
            name: "lace arm warmers",
            name_ko: "레이스 팔 토시",
            variable: "lacewarmers",
            post: 1,
            description: "패셔너블."
        },
		
        {
            index: 5,
            name: "long leather gloves",
            name_ko: "긴 가죽 장갑",
            variable: "longleathergloves",
            post: 0,
            description: "패셔너블하고 손을 주머니에 집어넣기 쉽다."
        },

        {
            index: 6,
            name: "cheerleader gloves",
            name_ko: "치어리더 장갑",
            variable: "pompoms",
            post: 0,
            description: "당신 팀의 승리를 응원하기 위한 것."
        },
 
        {
            index: 7,
            name: "gold bracelets",
            name_ko: "황금 팔찌",
            variable: "gold",
            post: 1,
            description: "이국적이고 시선을 끈다."
        },
 
        {
            index: 8,
            name: "cow sleeves",
            name_ko: "소 토시",
            variable: "cow",
            post: 1,
            description: "귀여운 소 발자국."
        },
 
        {
            index: 9,
            name: "work gloves",
            name_ko: "작업용 장갑",
            variable: "workgloves",
            post: 0,
            description: "손을 더럽혀야 한다면."
        },
 
        {
            index: 10,
            name: "sexy nun's gloves",
            name_ko: "섹시한 수녀 장갑",
            variable: "nunlewd",
            post: 0,
            description: "죄악을 문질러 지우기에 아주 좋다."
        },
 
        {
            index: 11,
            name: "wrist cuffs",
            name_ko: "손목 커프스",
            variable: "wristcuffs",
            post: 1,
            description: "플레이보이 바니걸이 착용하면 충분히 섹시해 보이고, 집사가 착용하면 충분히 세련되어 보인다."
        },
 
        {
            index: 12,
            name: "gold shackles",
            name_ko: "금 수갑",
            variable: "goldshackles",
            post: 0,
            description: "집념에 묶여 있다. 허영심에 묶여 있다."
        },

        {
            index: 13,
            name: "belted gloves",
            name_ko: "벨트 달린 장갑",
            variable: "belted",
            post: 0,
            description: "매일을 위한 버클."
        },

        {
            index: 14,
            name: "succubus gloves",
            name_ko: "서큐버스 장갑",
            variable: "goldshackles",
            post: 0,
            description: "마지막 한 방울까지 추출할 준비가 되었다."
        },
     ];
}
window.init_trHands = init_trHands;