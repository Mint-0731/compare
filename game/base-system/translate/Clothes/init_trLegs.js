function init_trLegs()
{
    setup.trClothes.legs = [
        {
            index: 0,
            name: "naked",
            name_ko: "벗음",
            variable: "naked",
            post: 0,
            description: "naked"
        },

        {
            index: 1,
            name: "tights",
            name_ko: "타이츠",
            variable: "tights",
            post: 1,
            description: "가리는게 많지는 않다."
        },

        {
            index: 2,
            name: "boy's gym socks",
            name_ko: "남성용 운동 양말",
            variable: "boysgymsocks",
            post: 2,
            description: "푹신푹신하고 공기가 잘 통한다. 학교에서 신기 적합하지만, 반드시 착용할 필요는 없다."
        },

        {
            index: 3,
            name: "girl's gym socks",
            name_ko: "여성용 운동 양말",
            variable: "girlsgymsocks",
            post: 2,
            description: "푹신푹신하고 공기가 잘 통한다. 학교에서 신기 적합하지만, 반드시 착용할 필요는 없다."
        },

        {
            index: 4,
            name: "stockings",
            name_ko: "스타킹",
            variable: "stockings",
            post: 0,
            description: "매력적이고 매혹적이다."
        },

        {
            index: 5,
            name: "fishnet stockings",
            name_ko: "망사 스타킹",
            variable: "fishnetstockings",
            post: 0,
            description: "매력적이고 매혹적이다."
        },

        {
            index: 6,
            name: "fishnet tights",
            name_ko: "망사 타이츠",
            variable: "fishnettights",
            post: 1,
            description: "매력적이고 매혹적이다."
        },

        {
            index: 7,
            name: "garter stockings",
            name_ko: "가터 스타킹",
            variable: "garterstockings",
            post: 0,
            description: "섹시하다."
        },

        {
            index: 8,
            name: "legwarmers",
            name_ko: "레그워머",
            variable: "legwarmers",
            post: 1,
            description: "아늑하다."
        },

        {
            index: 9,
            name: "christmas leg warmers",
            name_ko: "크리스마스 레그워머",
            variable: "christmas",
            post: 1,
            description: "축제 느낌이다."
        },

        {
            index: 10,
            name: "gold anklets",
            name_ko: "황금 발찌",
            variable: "goldanklets",
            post: 1,
            description: "이국적이고 시선을 끈다."
        },

        {
        index: 11,
            name: "naked",
            name_ko: "벗음",
            variable: "naked",
            post: 0,
            description: "naked"
        },

        {
            index: 12,
            name: "striped thighhighs",
            name_ko: "줄무늬 사이하이",
            variable: "stripedthighhighs",
            post: 1,
            description: "개성이 넘쳐흐른다."
        },

        {
            index: 13,
            name: "ankle socks",
            name_ko: "발목 양말",
            variable: "anklesocks",
            post: 2,
            description: "스포츠에 안성맞춤."
        },

        {
            index: 14,
            name: "plain thighhighs",
            name_ko: "평범한 사이하이",
            variable: "plainthighhighs",
            post: 1,
            description: "단순하고 귀엽다."
        },

        {
            index: 15,
            name: "cow socks",
            name_ko: "소 양말",
            variable: "cow",
            post: 2,
            description: "귀여운 소 발자국."
        },

        {
            index: 16,
            name: "nurse socks",
            name_ko: "간호사 양말",
            variable: "nursesocks",
            post: 2,
            description: "지역 병원의 간호사들이 사용한다."
        },

        {
            index: 17,
            name: "ribbon stockings",
            name_ko: "리본 스타킹",
            variable: "ribbonstockings",
            post: 0,
            description: "귀엽다."
        },

        {
            index: 18,
            name: "patterned dress socks",
            name_ko: "무늬가 들어간 신사 양말",
            variable: "twirly socks",
            post: 2,
            description: "남성용 신사 양말. 남성적이나, 쾌활하다."
        },

        {
            index: 19,
            name: "polka dot socks",
            name_ko: "물방울 무늬 양말",
            variable: "polka socks",
            post: 2,
            description: "무늬가 들어간 신사 양말. 남성적이나, 쾌활하다."
        },

        {
            index: 20,
            name: "sports socks",
            name_ko: "스포츠 양말",
            variable: "sports socks short",
            post: 2,
            description: "쿠션이 있고 통기성이 좋다."
        },

        {
            index: 21,
            name: "knee-high sports socks",
            name_ko: "무릎까지 오는 스포츠 양말",
            variable: "sports socks long",
            post: 2,
            description: "쿠션이 있고 통기성이 좋다."
        },

        {
            index: 22,
            name: "rib-knit socks",
            name_ko: "이랑 무늬 양말",
            variable: "striped socks long",
            post: 2,
            description: "즐거움을 선사하는, 남자용 이랑 무늬 양말."
        },

        {
            index: 23,
            name: "rib-knit ankle socks",
            name_ko: "이랑 무늬 발목 양말",
            variable: "striped socks short",
            post: 2,
            description: "즐거움을 선사하는, 남자용 이랑 무늬 양말."
        },

        {
            index: 24,
            name: "striped kneehighs",
            name_ko: "줄무늬 무릎 양말",
            variable: "striped kneehighs",
            post: 2,
            description: "개성으로 가득 차 있다."
        },

        {
            index: 25,
            name: "sheer leggings",
            name_ko: "속이 비쳐보이는 레깅스",
            variable: "sheerleggings",
            post: 1,
            description: "이것만 입기에는 너무 반투명하다."
        },

        {
            index: 26,
            name: "mismatched socks",
            name_ko: "짝짝이 양말",
            variable: "mismatched socks",
            post: 2,
            description: "개성이 넘친다."
        },

        {
            index: 27,
            name: "loose socks",
            name_ko: "루즈삭스",
            variable: "loosesocks",
            post: 1,
            description: "외국의 학생들이 즐겨 신는다."
        },

        {
            index: 28,
            name: "sexy nun's stockings",
            name_ko: "섹시한 수녀 스타킹",
            variable: "nunlewd",
            post: 0,
            description: "일요일 당신이 무릎을 꿇었을 때 가장 멋지게 보이게 하기 위해."
        },

        {
            index: 29,
            name: "leather leggings",
            name_ko: "가죽 레깅스",
            variable: "leatherleggings",
            post: 1,
            description: "너무나도 딱 달라붙어, 투명하다. (속옷의 가죽 레깅스를 구매하세요)"
        },

        {
            index: 30,
            name: "men's garter socks",
            name_ko: "남성용 가터 양말",
            variable: "mensgarters",
            post: 2,
            description: "섹시하고 실용적이다."
        },

        {
            index: 31,
            name: "ruffled socks",
            name_ko: "주름장식 양말",
            variable: "ruffled socks",
            post: 2,
            description: "주름장식이 붙어있다."
        },

        {
            index: 32,
            name: "ruffled kneehighs",
            name_ko: "주름장식 무릎 양말",
            variable: "ruffled kneehighs",
            post: 2,
            description: "주름장식이 붙어있다."
        },

        {
            index: 33,
            name: "ruffled thighhighs",
            name_ko: "주름장식 사이하이",
            variable: "ruffled thighhighs",
            post: 1,
            description: "주름장식이 붙어있다."
        },

        {
            index: 34,
            name: "tabi",
            name_ko: "다비",
            variable: "tabi",
            post: 1,
            description: "이국적이다."
        },

        {
            index: 35,
            name: "gold shackles",
            name_ko: "금 족쇄",
            variable: "goldshackles",
            post: 1,
            description: "세계의 무게로 묶여 있다."
        },

        {
            index: 36,
            name: "mismatched thighhighs",
            name_ko: "짝짝이 사이하이",
            variable: "mismatched thighhighs",
            post: 1,
            description: "마치 어둠 속에서 옷을 입은 것 같다."
        },

        {
            index: 37,
            name: "sheer stockings",
            name_ko: "시스루 스타킹",
            variable: "sheer stockings",
            post: 0,
            description: "투명하다."
        },

        {
            index: 38,
            name: "belted thighhighs",
            name_ko: "벨트 달린 사이하이",
            variable: "belted",
            post: 1,
            description: "매일을 위한 벨트."
        },

        {
            index: 39,
            name: "leather thighhighs",
            name_ko: "가죽 사이하이",
            variable: "leather",
            post: 1,
            description: "반투명함이 당신의 종아리를 강조한다."
        },

        {
            index: 40,
            name: "chain leggings",
            name_ko: "사슬 레깅스",
            variable: "chain leggings",
            post: 1,
            description: "신체에서 가장 취약한 동맥을 보호한다. 아니다, 경정맥은 정맥이다."
        },
    ];
}
window.init_trLegs = init_trLegs;