function init_trFace()
{
    setup.trClothes.face = [
        {
            index: 0,
            name: "naked",
            name_ko: "벗음",
            variable: "naked",
            post: 0,
            description: "naked"
        },

        {
            index: 1,
            name: "glasses",
            name_ko:"안경",
            variable: "glasses",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 2,
            name: "cool shades",
            name_ko: "멋진 선글라스",
            variable: "coolshades",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 3,
            name: "surgical mask",
            name_ko: "수술용 마스크",
            variable: "surgicalmask",
            post: 1,
            description: "신원을 가려준다."
        },

        {
            index: 4,
            name: "gag",
            name_ko: "재갈",
            variable: "gag",
            post: 2,
            description: "입을 다물게 만든다."
        },

        {
            index: 5,
            name: "blindfold",
            name_ko: "안대",
            variable: "blindfold",
            post: 1,
            description: "시야를 가려버린다."
        },

        {
            index: 6,
            name: "gag and blindfold",
            name_ko: "재갈과 안대",
            variable: "gagblindfold",
            post: 1,
            description: "입과 눈을 모두 막아버린다."
        },

        {
            index: 7,
            name: "skulduggery mask",
            name_ko: "도둑 마스크",
            variable: "skulmask",
            post: 1,
            description: "나쁜 짓을 꾸미는 이들을 위한 물건."
        },

        {
            index: 8,
            name: "low frame glasses",
            name_ko: "로우 프레임 안경",
            variable: "lowframe",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 9,
            name: "half moon glasses",
            name_ko: "하프문 안경",
            variable: "halfmoon",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 10,
            name: "deep frame glasses",
            name_ko: "딥 프레임 안경",
            variable: "deepframe",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 11,
            name: "square shades",
            name_ko: "사각 선글라스",
            variable: "square",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 12,
            name: "round shades",
            name_ko: "둥근 선글라스",
            variable: "round",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 13,
            name: "shield shades",
            name_ko: "쉴드 선글라스",
            variable: "shield",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 14,
            name: "cat eye shades",
            name_ko: "캣아이 선글라스",
            variable: "cateye",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 15,
            name: "aviators",
            name_ko: "에비에이터",
            variable: "aviator",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 16,
            name: "punk shades",
            name_ko: "펑크 선글라스",
            variable: "punk",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 17,
            name: "muzzle",
            name_ko: "입마개",
            variable: "muzzle",
            post: 1,
            description: "당신의 그 위험한 주둥이를 다물게 만든다."
        },

        {
            index: 18,
            name: "mummy facewrap",
            name_ko: "미라 얼굴 붕대",
            variable: "mummy",
            post: 1,
            description: "화장실 휴지로 만든 것 같다."
        },
        
        {
            index: 19,
            name: "swimming goggles",
            name_ko: "수영 고글",
            variable: "swimgoggles",
            post: 2,
            description: "아래에서 시야를 확보해준다."
        },
        
        {
            index: 20,
            name: "belly dancer's veil",
            name_ko: "밸리댄서의 베일",
            variable: "belly",
            post: 2,
            description: "이국적이고 유혹적이다."
        },
        
        {
            index: 21,
            name: "muzzle",
            name_ko: "입마개",
            variable: "muzzlefetish",
            post: 1,
            description: "당신의 그 위험한 주둥이를 다물게 만든다."
        },
        
        {
            index: 22,
            name: "gag",
            name_ko: "재갈",
            variable: "gagfetish",
            post: 2,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 23,
            name: "bit gag",
            name_ko: "말 재갈",
            variable: "bitgag",
            post: 2,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 24,
            name: "wolf muzzle",
            name_ko: "늑대 입마개",
            variable: "wolfmuzzle",
            post: 1,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 25,
            name: "kitty muzzle",
            name_ko: "고양이 입마개",
            variable: "kittymuzzle",
            post: 1,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 26,
            name: "cloth gag",
            name_ko: "천 재갈",
            variable: "clothgag",
            post: 2,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 27,
            name: "panty gag",
            name_ko: "팬티 재갈",
            variable: "pantygag",
            post: 2,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 28,
            name: "tape gag",
            name_ko: "테이프 재갈",
            variable: "tapegag",
            post: 2,
            description: "입을 다물게 만든다."
        },
        
        {
            index: 29,
            name: "penis gag",
            name_ko: "자지 모양 재갈",
            variable: "penisgag",
            post: 2,
            description: "입을 꽉 차게 만든다."
        },

        {
            index: 30,
            name: "skeleton mask",
            name_ko: "해골 마스크",
            variable: "skele",
            post: 1,
            description: "당신이 뼈다귀를 선택한다면."
        },

        {
            index: 31,
            name: "esoteric spectacles",
            name_ko: "비전의 안경",
            variable: "esoteric",
            post: 0,
            description: "물담배 마스터의 안경."
        },

        {
            index: 32,
            name: "gas mask",
            name_ko: "가스 마스크",
            variable: "gasmask",
            post: 1,
            description: "신원을 가려주지만, 이상한 모습으로 보일 것이다."
        },

        {
            index: 33,
            name: "eyepatch",
            name_ko: "안대",
            variable: "eyepatch",
            post: 1,
            description: "대양을 항해하려 한다면."
        },

        {
            index: 34,
            name: "medical eyepatch",
            name_ko: "의료용 안대",
            variable: "medical eyepatch",
            post: 1,
            description: "위급할 때에는."
        },

        {
            index: 35,
            name: "doggy muzzle",
            name_ko: "개 입마개",
            variable: "doggymuzzle",
            post: 1,
            description: "당신의 그 위험한 주둥이를 다물게 만든다."
        },

        {
            index: 36,
            name: "bamboo muzzle",
            name_ko: "대나무 입마개",
            variable: "bamboomuzzle",
            post: 1,
            description: "당신의 그 위험한 주둥이를 다물게 만든다."
        },

        {
            index: 37,
            name: "monocle",
            name_ko: "단안경",
            variable: "monocle",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 38,
            name: "islander mask",
            name_ko: "섬사람 마스크",
            variable: "islander mask",
            post: 1,
            description: "섬사람이 쓴다."
        },

        {
            index: 39,
            name: "reading glasses",
            name_ko: "독서 안경",
            variable: "reading",
            post: 0,
            description: "공부 하기가 수월하지만, 학교에서 놀림당할 수도 있다."
        },

        {
            index: 40,
            name: "bandanna",
            name_ko: "반다나",
            variable: "bandanna",
            post: 1,
            description: "신원을 가려준다."
        },

        {
            index: 41,
            name: "heart sunglasses",
            name_ko: "하트모양 선글라스",
            variable: "heartsunglasses",
            post: 1,
            description: "학교에서 평판이 빠르게 상승한다."
        },

        {
            index: 42,
            name: "fox mask",
            name_ko: "여우 마스크",
            variable: "foxmask",
            post: 1,
            description: "엘크의 선율에 장단을 맞춰 춤출 때 사용한다."
        },
        
        {
            index: 43,
            name: "futuristic shades",
            name_ko: "미래지향적인 셰이드",
            variable: "futureshades",
            post: 1,
            description: "세련된 로봇 같음."
        },
	];
}
window.init_trFace = init_trFace;