...
function masturbationAudience() {
	...
	if (V.masturbationAudience >= 1) {
		...
		if (V.npc[npc - 1]) {
			sWikifier(
				`<span class="lewd"><<person${npc}>>당신은 남자의 눈길이${V.masturbationAudience > 1 ? ", 다른 사람들과 같이," : ""} 당신을 향하고 있다는 것을 느낀다.${
					audienceMutual && V.masturbationAudience <= 6
						? ` 그는 당신의 움직임을${V.mouth === 0 || V.mouth === "disabled" ? " " : "일부 "} 따라하고 있다.`
						: ""
				}</span>`
			);
		} else if (V.masturbationAudience === 1) {
			sWikifier(
				`<span class="lewd"><<person${npc}>>당신은 남자의 눈길이 당신을 향하고 있${
					audienceMutual ? `으며 그가 당신의 움직임을${V.mouth === 0 || V.mouth === "disabled" ? " " : "일부 "} 따라하고 있다는 것을 느낀다` : "다는 것을 느낀다"
				}.</span>`
			);
...
function masturbationAudienceLineText(npc, lineType = "") {
	...
	switch (lineType + namedNpc) {
		case "orgasm":
			...
			if (V.masturbationAudience > 1) {
				resultArray.push(`"그녀가 얼마나 몸을 많이 떨고 있는지 봐. 꽤나 강렬한 절정이었을 게 분명해."`);
			}
			...
			if (V.masturbationAudienceReactions.filter(a => a === "orgasm").length > 2) {
				if (V.masturbationAudience > 1) {
					resultArray.push(`"이 걸레는 쉬지도 않고 계속 가버리네!"`);
				}
				resultArray.push(`"그렇게 자주 가버리는 건 어떤 느낌일까!"`);
			}
			if (V.masturbationorgasm >= 5) {
				if (V.masturbationAudience > 1) {
					resultArray.push(`"이 걸레가 ${V.masturbationAudience >= 4 ? "모두가" : "우리가"} 보는 앞에서 계속 가버리고 있다고?"`);
				} else {
		...
		case "chastitypenis":
			...
			if (V.masturbationAudience >= 4) {
				resultArray.push(
					`"그녀가 그걸 얼마나 세게 쥐어짜는지 보라고."`,
					`"너 정말 기분 좋은 것 처럼 보이네. 그러니까 너 정말 걸레 같아 보여."`,
					`"바로 그거야, 모두에게 네가 어떻게 자위하는지 보여주라고."`
				);
			} else {
				resultArray.push(
		...
		case "vagina":
			if (V.masturbationAudience >= 4) {
				resultArray.push(
					...
					`"그녀의 클리토리스가 정말 딱딱해졌어."`,
					`"그래 소녀야, 모두에게 네가 보지로 어떻게 자위하는지 보여주라고."`
				);
			} else {
				resultArray.push(
					`"네가 얼마나 젖었는지 좀 봐."`,
					...
					`"네 클리토리스가 정말 딱딱해졌어."`,
					`"그래 소녀야, ${V.masturbationAudience > 1 ? "우리에게" : "내게"} 네가 보지로 어떻게 자위하는지 보여주라고."`
				);
			}
			if (V.fingersInVagina <= 0) resultArray.push(`"이봐, 손가락들 좀 넣어보라고."`);
			if (between(V.fingersInVagina, 1, 3))
				resultArray.push(
					`"그게 ${V.masturbationAudience > 1 ? "그녀가" : "네가"} ${
						V.masturbationAudience > 1 ? " 자신 안에 넣을 수 있는 손가락 전부야? 처녀인가 보네." : "넣을 수 있는 손가락 전부야?"
					}"`,
			...
			if (V.fingersInVagina >= 5) {
				resultArray.push(
					`"${V.masturbationAudience > 1 ? "그녀가" : "네가"} ${
						V.masturbationAudience > 1 ? "그녀의" : ""
					} 주먹 전체를 그 안에 넣을 수 있을 줄은 몰랐는데, 내가 틀렸나 보네. 정말 많이 들어가는군."`
				);
			}
			if (V.fingersInVagina >= 1) resultArray.push(`"네 보지에서 나오는 음란한 소리가 우리 모두에게 들리고 있어, 소녀야.`);
			return resultArray.random();
		case "anusEntrance":
			resultArray.push(
				`"${V.masturbationAudience > 1 ? "그녀가" : "네가"} ${V.masturbationAudience > 1 ? "그녀의" : "네"} 엉덩이를 만지는 게 잘 보여."`,
				`"이렇게 때려보고 싶은 엉덩이를 지금까지 한 번도 본 적이 없어."`,
				`"이봐, 손가락 좀 넣어 봐."`,
				`"${V.masturbationAudience > 1 ? "우리에게" : "내게"} 네가 어떻게 엉덩이로 자위하는지 보여주라고, 걸레야."`
			);
		...
		case "anus":
			return [
				...
				`"항문으로 자위하는 게 그렇게 좋아? 걸레 같으니라고."`,
				`"${V.masturbationAudience > 1 ? "그녀는" : "너는"} 그 항문으로 경험을 많이 쌓았던 게 틀림없어."`,
				`"네가 손가락보다 더한 것을 원한다면, 내가 도와주지."`,
				`"제기랄, ${V.masturbationAudience > 1 ? "그녀가" : "네가"} 진짜로 ${
					V.masturbationAudience > 1 ? "그녀의" : "네"
				} 주먹을 안에 집어넣을 줄이야. 그 항문에 지금까지 얼마나 많은 것들이 들어갔었던 거야?"`,
			].random();
		case "mouthBreast":
			resultArray.push(
				`"자신의 젖꼭지를 빨고 있다니, 멍청한 걸레 같으니라고."`,
				`"자산의 커다란 젖가슴을 빠는 것이 얼마나 쉬운지 과시하고 있는 거야? 너 정말 변태구나."`,
				`"나도 그 젖꼭지들 좀 빨아도 될까?"`
			);
			if (V.lactating && V.settings.breastFeedingEnabled === true) {
				resultArray.push(`"그녀가 얼마나 열심히 자신의 젖을 짜내고 있는지 좀 봐."`);
			}
			return resultArray.random();
		case "chest":
			resultArray.push(
				`"그 젖꼭지 좀 쥐어짜봐, 걸레야."`,
				`"${V.masturbationAudience > 1 ? "그녀의" : "네"} 젖꼭지가 얼마나 딱딱해졌는지 좀 봐."`,
		...
		case "penisSize0":
			return [
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지는 정말 조그마하구나!"`,
				`"그게 저렇게 작다니 믿을 수가 없어!"`,
				`"저런 한심한 자지는 지금까지 본 적이 없어."`,
			]
				.random()
		...
		case "penisSize1":
			return [
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지는 정말 작구나!"`,
				`"정말 작고, 귀여운 자지네."`,
				`"나는 ${V.masturbationAudience > 1 ? "그의" : "네"} 자지가 더 클 거라 생각했는데."`,
			]
				.random()
				.concat('<<ginsecurity "penis_small">>');
		case "penisSize2":
			return [
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 귀여운 자지 사진을 찍었으면 좋겠어."`,
				`"부끄러워하지 마, 모두가 네 자지가 얼마나 아름다운지 알아야 한다고."`,
				`"네 자지 사진이 찍히고 싶지 않다면, 걸레처럼 행동하지 않는 게 좋을 거야."`,
			].random();
		case "penisSize3":
			return [
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지는 내 생각보다 큰데."`,
				`"부끄러워하지 마, 너는 네 자지에 대해 자랑스러워해야 한다고."`,
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지 크기는 완벽해."`,
			]
				.random()
				.concat(V.player.gender !== "m" ? `<<ginsecurity "penis_big">>` : "");
		case "penisSize4":
			return [
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지 정말 크다!"`,
				`"${V.masturbationAudience > 1 ? "그의" : "네"} 자지는 희한하게 크네."`,
		...
		case "breastSizeFlat":
			if (V.player.gender_appearance === "m") {
				...
				if (V.worn.upper.exposed >= 2) {
					return [
						`"네 평평한 가슴은 맛있어 보이네."`,
						`"${V.masturbationAudience > 1 ? "그녀의" : "네"} 가슴은 정말 평평해서, ${
							V.masturbationAudience > 1 ? "그녀가" : "네가"
						} 남자라고 해도 믿을 것 같아."`,
						`"내가 그녀의 귀여운 가슴을 찍어놓는 게 좋을까? 나중에 유용해질 것 같은데."`,
					]
						.random()
		...
		case "breastSizeSmall":
			if (V.player.gender_appearance === "m") {
				...
				if (V.worn.upper.exposed >= 2) {
					...
					return [
						`"그녀의 윗옷 아래로 그녀의 조그마한 젖가슴의 윤곽이 보여."`,
						`"윗도리 좀 벗어 봐, 네 젖가슴을 더 잘 보고 싶다고."`,
						`"그녀의 작은 젖가슴을 보고 싶어, 분명 훌륭할 거야."`,
					]
						.random()
						.concat(`<<ginsecurity "breasts_small">>`);
				}
		...
		case "breastSizeNormal":
			if (V.worn.upper.exposed >= 2) {
				...
				return [
					`"${V.masturbationAudience > 1 ? "그녀의 윗도리 좀 벗겨" : "네 윗도리 좀 벗어"} 봐, ${
						V.masturbationAudience > 1 ? "그녀의" : "네"
					} 젖가슴을 더 잘 보고 싶다고."`,
					`"부끄러워하지 마, 네 젖가슴이 아직 드러난 것도 아니잖아."`,
					`"옷으로 가려져 있어도, 나는 ${V.masturbationAudience > 1 ? "그녀의" : "네"} 젖가슴이 얼마나 사랑스러운지 알겠어."`,
				]
					.random()
					.concat(V.player.gender === "m" ? `<<ginsecurity "breasts_big">>` : "");
			}
		case "breastSizeLarge":
			if (V.worn.upper.exposed >= 2) {
				return [
					`"${V.masturbationAudience > 1 ? "그녀의" : "네"} 젖가슴이 정말 아름답게 흔들리네."`,
					`"그거 정말 인상적인 젖가슴이잖아."`,
				...
				return [
					`"${V.masturbationAudience > 1 ? "그녀의" : "네"} 젖가슴이 정말 보이는 것 만큼 커다란 거야? 알아볼 방법은 하나밖에 없지."`,
