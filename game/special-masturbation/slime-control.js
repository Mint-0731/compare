...
function masturbationSlimeControl() {
	...
	if (alternateForcedActions.includes("selfImpreg")) {
		...
		fragment.append(Wikifier.wikifyEval('<span class="red">슬라임이 당신의 입을 페니스에서 치우지 못하게 한다.</span>'));
		fragment.append(" ");
		V.mouthaction = "slime";
	} else if (V.mouthaction === "mchastityparasitestop") {
		fragment.append(Wikifier.wikifyEval('<span class="red">슬라임이 당신의 입을 기생충 정조대의 페니스에서 치우지 못하게 한다.</span>'));
		fragment.append(" ");
		V.mouthaction = "slime";
	} else if (V.mouthaction === "mpenispullback") {
		fragment.append(
			Wikifier.wikifyEval('<span class="red">슬라임이 지금 하는 것처럼 당신의 페니스를 깊이 집어넣고 빨고 있는 중에 당신이 머리를 뒤로 빼내지 못하게 한다.</span>')
		);
		fragment.append(" ");
		V.mouthaction = "slime";
	} else if (V.mouthaction === "mvaginastop") {
		fragment.append(redText("슬라임이 당신의 입을 질에서 치우지 못하게 한다."));
		fragment.append(" ");
		V.mouthaction = "slime";
	}
