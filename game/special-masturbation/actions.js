...
function masturbationActionsHands(arm, { playerToys, selectedToy, toyDisplay, genitalsExposed, breastsExposed, ballsExposed }) {
	...
	switch (V[arm + "arm"]) {
		case 0:
			result.text = `당신은 ${armName[arm]} 손으로 아무것도 하지 않고 있다.`;
			...
			break;
		case "mpenisentrance":
			result.text = `당신은 ${V.player.penissize >= 0 ? `${armName[arm]} 손으로.` : `${armName[arm]} 엄지와 손가락으로.`} 페니스를 잡고 있다.`;
			if (V.mouth !== "mpenis") {
				result.options.push({
					action: "mpenisglans",
					text: "귀두를 애무한다",
		...
		case "mchastityparasiteentrance":
			result.text = `당신은 ${V.player.penissize >= 2 ? `${armName[arm]} 손으로` : `${armName[arm]} 엄지와 손가락으로`} 기생충 정조대 페니스를 잡고 있다.`;
			if (V.mouth !== "mpenis") {
				result.options.push({
		...
		case "mvaginaentrance":
			result.text = `당신은 ${armName[arm]} 손으로 질을 문지르고 있다.`;
			if (genitalsExposed()) {
		...
		case "mvaginaentrancedildo":
			if (!selectedToy(arm)) {
				result.text = `당신이 섹스 장난감을 선택하지 않아서 당신의 ${armName[arm]} 손은 비어있다.`;
				result.options.push(stop("mvaginastopdildo"));
			} else if (playerChastity("vagina")) {
				result.text = `당신의 정조대가 질을 막고 있다.`;
				result.options.push(stop("mvaginastopdildo"));
			} else {
				result.text = `당신은 ${armName[arm]} 손의 ${toyDisplay(selectedToy(arm))}로 질을 비벼댄다.${
			...
			break;
		case "mvaginadildo":
			result.text = `당신은 ${armName[arm]} 손의 ${toyDisplay(selectedToy(arm))}로 당신의 질을 찔러대고 있다.`;
			result.options.push({
			...
			break;
		case "mbreasthold":
			result.text = `당신은 ${armName[arm]} 손으로 ${armName[arm]} 가슴을 잡고 있다.`;
			result.options.push({
			...
			break;
		case "manusentrance":
			result.text = `당신은 ${armName[arm]} 손으로 항문을 희롱하고 있다.`;
		...
		case "manusentrancedildo":
			result.text = `당신은 ${armName[arm]} 손의 ${toyDisplay(selectedToy(arm))}로 당신의 항문을 희롱하고 있다.`;
			...
			break;
		case "manusdildo":
			result.text = `당신은 ${armName[arm]}의 ${toyDisplay(selectedToy(arm))}로 당신의 항문을 희롱하고 있다.`;
			result.options.push({
		...
		case "mpenisentrancestroker":
			result.text = `당신은 ${armName[arm]} 손의 ${toyDisplay(selectedToy(arm))}로 귀두를 희롱하고 있다.`;
			if (genitalsExposed()) {
				result.options.push({
		...
		case "mdildomouthentrance":
			result.text = `당신은 ${armName[arm]} 손의 ${toyDisplay(selectedToy(arm))}를 당신의 입에 대고 있다.`;
			result.options.push({
				action: "mdildomouth",
				text: "입 안에 집어넣는다",
				colour: "sub",
			...
			break;
		case "mdildomouth":
			result.text = `당신은 ${armName[arm]} 손으로 입 안의 ${toyDisplay(selectedToy(arm))}를 잡고 있다.`;
			result.options.push({
				action: "mdildopiston",
				text: "앞뒤로 움직인다",
				colour: "sub",
			...
			break;
		case "mpickupdildo":
			result.text = `당신은 ${armName[arm]} 손으로 ${toyDisplay(selectedToy(arm))}를 잡고 있다.`;
			...
			break;
		case "mballs":
			result.text = `당신은 자신의 고환${V.ballssize >= 1 ? " 중 하나를" : "을"} 당신의  ${armName[arm]} 손으로 잡고 있다.`;
			result.options.push({
				action: "mballsfondle",
				text: "애무한다",
				colour: "sub",
			...
			break;
		case "bound":
			result.text = `당신의 ${armName[arm]} 손은 묶여있다.`;
			break;
		case "possessed":
			if (V.lactating && V.settings.breastFeedingEnabled === true && arm === "right") {
				result.text =
					actiondefault === "mbreastW"
						? `당신은 당신의 의사와는 무관하게, ${armName[arm]} 손으로 <<breasts>>을 꼬집거나 짓누르고 있다.`
						: `당신의 ${armName[arm]} 손은 <<breasts>> 위에 떠 있다.`;
				result.options.push({
					action: "mbreastW",
					text: "가슴을 애무한다",
					colour: "wraith",
				...
				if (V.worn.genitals.name === "chastity parasite") {
					result.text =
						actiondefault === "mpenisW"
							? `당신은 당신의 의사와는 무관하게, ${armName[arm]} 손으로 기생충 정조대 페니스를 희롱하고 있다.`
							: `당신의 ${armName[arm]} 손은 기생충 정조대 페니스 위에 떠 있다.`;
					result.options.push({
						action: "mpenisW",
						text: "기생충 페니스를 비빈다",
						colour: "wraith",
					...
					result.text =
						actiondefault === "mpenisW"
							? `당신은 당신의 의사와는 무관하게, ${armName[arm]} 손으로 페니스를 쓰다듬고 있다.`
							: `당신의 ${armName[arm]} 손은 페니스 위에 떠 있다.`;
					result.options.push({
						action: "mpenisW",
						text: "육봉을 비빈다",
						colour: "wraith",
				...
				result.text =
					actiondefault === "mvaginaW"
						? `당신은 당신의 의사와는 무관하게, ${armName[arm]} 손으로 질을 쓰다듬고 있다.`
						: `당신의 ${armName[arm]} 손은 질 위에 떠 있다.`;
				result.options.push({
					action: "mvaginaW",
					text: "질을 애무한다",
					colour: "wraith",
