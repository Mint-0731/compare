...
/* following line is minified version of https://github.com/preactjs/preact used to handle reactive ui updates. */
...
function redrawImg(tmp_object, saved_index, remember_eye) {
	setTimeout(() => {
		/*console.log("debug 4:")
		console.log("setup.colours.eyes = ")
		console.log(setup.colours.eyes)
		console.log("left eye = "+V.leftEyeColour+ " right eye ="+V.rightEyeColour)*/
		...
		console.log(saved_index)*/
		for (let eye of [0,1]){
			t_blend = [tmp_object[""+eye]["normal"]["blend"], tmp_object[""+eye]["custom"]["blend"]];
			t_brightness = [tmp_object[""+eye]["normal"]["brightness"], tmp_object[""+eye]["custom"]["brightness"]];
			setup.colours.eyes[saved_index[eye]].canvasfilter.blend = t_blend[0];
			if (t_brightness[0] != undefined)
				setup.colours.eyes[saved_index[eye]].canvasfilter.brightness = t_brightness[0];
			if (t_blend[1] != undefined){
				let eye2 = tmp_object[""+eye]["custom"]["index"]
				V.custom_eyecolours[eye2].canvasfilter.blend = t_blend[1]
				if (t_brightness[2] != undefined)
					V.custom_eyecolours[eye2].canvasfilter.brightness = t_brightness[1]
				//console.log("debug 5 :")
				tempEditEyeName(eye, remember_eye[eye], eye2);
			}
			else{
				//console.log("debug 6 :")
				tempEditEyeName(eye, remember_eye[eye], saved_index[eye]);
			}
		}
...
function tempEditEyeName(index, value, eye){
	...
	if (V.makeup.eyelenses.left != 0 && index == 0)
		...
		V.leftEyeColour = value
	if (V.makeup.eyelenses.right != 0 && index == 1)
		V.makeup.eyelenses.right = value
	else if (V.makeup.eyelenses.right == 0 && index == 1)
		V.rightEyeColour = value
}

function tempEditPlayerEyeValues(tmp_save, eye, eye_name, color, s_c){
	tmp_save[""+eye]["normal"] = {"blend": setup.colours.eyes[s_c].canvasfilter.blend, /* save current eye blend/brightness values to temp variable.*/
	"brightness":setup.colours.eyes[s_c].canvasfilter.brightness}
	for (let s_colours in setup.colours.eyes){
		if (setup.colours.eyes[s_colours].variable == eye_name){
			setup.colours.eyes[s_colours].canvasfilter.blend = color.hexString
			setup.colours.eyes[s_colours].canvasfilter.brightness = color.hsl["l"] <= 0 ? 0 : color.hsl["l"] / 350
			if (eye_name == "colorWheelTemporary0" || eye_name == "colorWheelTemporary1")
				setup.colours.eyes[s_colours].csstext = window.colorNamer(color.hexString)
			/*console.log("debug 0========")
			console.log("setup.colours.eyes["+s_colours+"].canvasfilter.blend = "+color.hexString)
			console.log("setup.colours.eyes["+s_colours+"].canvasfilter.brightness = "+(color.hsl["l"] <= 0 ? 0 : color.hsl["l"] / 350))
			console.log("========================")
			console.log(tmp_save)*/
			break
		}
	}
	/* custom eye colours also need to be modified */
	for (let c_color in V.custom_eyecolours){
		if (V.custom_eyecolours[c_color].variable == V.makeup.eyelenses){
			tmp_save[""+eye]["custom"] = {"blend": V.custom_eyecolours[c_color].canvasfilter.blend,
								  "brightness": V.custom_eyecolours[c_color].canvasfilter.brightness,
								  "index": c_color}
			V.custom_eyecolours[c_color].canvasfilter.blend = color.hexString
			V.custom_eyecolours[c_color].canvasfilter.brightness = color.hsl["l"] <= 0 ? 0 : color.hsl["l"] / 350
			break;
		}
		c_color = undefined
	}
...
window.colorWheelContactLenses = function() {
	...
	$(function(){
		...
		var colorPicker = new iro.ColorPicker('#picker', {
			colors: (V.pharmacy_order_colours == 2 ?
			[
				p_eyes[0], // player left eye colour
				p_eyes[1], // player right eye colour*/
			] :[
				p_eyes[0]
			]),
			layout: [
